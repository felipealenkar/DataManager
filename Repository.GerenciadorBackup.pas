unit Repository.GerenciadorBackup;
...
type
  ...
  TGerenciadorBackup = class (TObject)
    ...
    public
      ...
      Senha: string;
      Extensao: string;
      TipoArquivoDlg: string;
      Versao: currency;
    const
      PgVersaoMinima: Currency = 17;
    ...
    function ValidarOwnerDatabase(PNomeDoDatabase, POwnerPadrao: string; out POwnerBD: string): Boolean;
...
begin
  ...
  Extensao := 'postgresql';
  TipoArquivoDlg := 'Backup PostgreSQL';
  Versao := PVersaoPg;
  if PDatabase <> '' then Database := PDatabase;

  Driver := 'PG';
  Biblioteca := 'C:\Program Files\PostgreSQL\' + Versao.ToString + '\bin\libpq.dll';
...
begin
  ...
  try
    Query.close;
    Query.sql.Clear;
    Query.sql.Add('DROP ROLE "' + OwnerPadrao + '"');
    Query.ExecSQL;
  except
    On E: Exception do
    begin
  ...
  if Falha > 0 then
    RegistrarLogs('TGerenciadorBackup.DroparRoles', Mensagem)
  else
    RegistrarLogs('TGerenciadorBackup.DroparRoles', 'Usuários padrão do database "' + PNomeDoDatabase + ' Excluídos com sucesso.');
end;
...
begin
  ...
  if Falha > 0 then
    RegistrarLogs('TGerenciadorBackup.RemoverPermissoesNosRoles', Mensagem)
  else
    RegistrarLogs('TGerenciadorBackup.RemoverPermissoesNosRoles', 'Permissões removidas  dos usuários do database "' + PNomeDoDatabase + '".');
end;
...
begin
  try
    Query.close;
    Query.sql.Clear;
    Query.sql.add('ALTER DATABASE "' + PNomeDoDatabaseAntigo + '" RENAME TO "' + PNomeDoDatabaseNovo + '"');
    Query.ExecSQL;
    Query.close;
    RegistrarLogs('TGerenciadorBackup.RenomearDatabase', 'Database "' + PNomeDoDatabaseAntigo + '" renomeado para "' + PNomeDoDatabaseNovo + '" com sucesso');
  except
...
procedure TGerenciadorBackup.SelectTodosDatabases;
begin
  try
    Query.close;
    Query.sql.Clear;
    Query.SQL.text := 'select datname from pg_database where datistemplate = false and datname <> ''postgres'' order by datname';
    Query.Open;
  except
    On E: Exception do
    begin
      RegistrarLogs('TGerenciadorBackup.SelectTodosDatabases', E.ClassName + ' ' + E.Message);
      raise;
    end;
  end;
end;
