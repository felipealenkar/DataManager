...
type
  ...
  public
    ...
    constructor Create(const Cmd, Pwd, AOutputFilePathParam, LogPath: string; IsDump: Boolean; ProgressBar: TProgressBar;
                       LblStatus: TLabel; RichEditLog: TRichEdit; Connection: TFDConnection; DumpRestorePath: string);
  ...
  public
    ...
    procedure IniciarOperacao(const ACommand: string; const AOutputFilePath: string;
                              ...
                              ANomeDoDatabase, ASenha: string; AAcao: TEnumAcao;
    ...
    property PGAcao: TEnumAcao read FPAcao_Type write FPAcao_Type;
...
begin
  ...
  RegistrarLogs('TFormBackupRestore.FormShow', 'Backup do banco de dados "' + FPNomeDoDatabase_Name + '" Iniciado');
  ...
  // O comando completo
  OutputDebugString(PChar('DEBUG(FormShow): FCommandString (from IniciarOperacao) = ' + FCommandString));
...
begin
  ...
  begin
    ...
    begin
      ...
      StatusMessage := 'Despejando dados da tabela: ' + ExtractTableNameFromLog(Line) +
                        Format(' (%d/%d)', [FProcessedItems, FTotalItemsToProcess]);
    end
  ...
  begin
    ...
    begin
      ...
      StatusMessage := 'Restaurando dados da tabela: ' + ExtractTableNameFromLog(Line) + Format(' (%d/%d)',
                        [FProcessedItems, FTotalItemsToProcess]);
      OutputDebugString(PChar(Format('DEBUG(Restore Data Phase): Line="%s", FProcessedItems=%d, FTotalItemsToProcess=%d,' +
                        'CalculatedPosition=%d', [Line, FProcessedItems, FTotalItemsToProcess, CurrentPosition])));
    end
...
// ===========================================================================
function TPostgreSQLWorkerThread.CountObjectsInDumpFile(const ADumpFile: string; const PgRestorePath: string;
  ...
  try
    ...
    try
      ...
      try
        ...
        LHandleStream := THandleStream.Create(StdOutPipeRead);
        OutputReader := TStreamReader.Create(LHandleStream);
...
begin
  ...
  try
    ...
    begin
      ...
      begin
          UpdateUI(RESTORE_PHASE_ESTIMATING_END, Format('Estimativa: %d objetos de dados para restaurar.',
                                                        [FTotalItemsToProcess]), '');
      end;
    end;
